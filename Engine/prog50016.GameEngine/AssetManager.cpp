...
void AssetManager::load(XMLElement* element, STRCODE fileID)
{
	...
	while (assetElement != NULL)
	{
		...
		std::shared_ptr<Asset> asset((Asset*)CreateObject(assetType.c_str()));
...
std::shared_ptr<Asset> AssetManager::getAsset(std::string AssetUID, STRCODE Type)
{
	return getAsset(getHashCode(AssetUID.c_str()), Type);
}

std::shared_ptr<Asset> AssetManager::getAsset(STRCODE assetUID, STRCODE Type)
{
	std::map<STRCODE, std::shared_ptr<Asset>>::iterator it = assets.find(assetUID);
	if (it != assets.end())
	{
		if (Type == NoName)
		{
